...
from utils import DINOv2Classifier, add_pseudo_labels, split_by_base_image
...
def train(seed):
    ...
    model.fit_generator(
        ...
        val_loader_head,
        epochs=EPOCHS_HEAD,
        callbacks=build_callbacks(f"dinov2-evolved-s{seed}-head.pt", PATIENCE_HEAD, EPOCHS_HEAD),
    )
    model.load_weights(f"dinov2-evolved-s{seed}-head.pt")
    ...
    model.fit_generator(
        ...
        callbacks=build_callbacks(f"dinov2-evolved-s{seed}-partial.pt", PATIENCE_PARTIAL, EPOCHS_PARTIAL),
    )
    model.load_weights(f"dinov2-evolved-s{seed}-partial.pt")
    ...
    model.fit_generator(
        ...
        val_loader_final,
        epochs=EPOCHS_FINAL,
        callbacks=build_callbacks(f"dinov2-evolved-s{seed}-final.pt", PATIENCE_FINAL, EPOCHS_FINAL),
    )
    model.load_weights(f"dinov2-evolved-s{seed}-final.pt")
    print(f"Checkpoint final: dinov2-evolved-s{seed}-final.pt")
